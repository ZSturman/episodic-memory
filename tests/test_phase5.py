...
class TestDeltaDetector:
    ...
    def test_detects_new_entity(self, delta_detector, sample_acf):
        ...
        assert deltas2[0].entity_category == "ball"
...
class TestEventResolverStateChange:
    ...
    def test_creates_opened_event(self, event_resolver, sample_acf):
        """Test that opening a drawer creates an OPENED event."""
        # First percept: drawer closed
        percept1 = make_percept([{
            "guid": "drawer_001",
            "label": "Drawer",
        ...
        # Should have an OPENED event
        assert len(events2) == 1
        event = events2[0]
        assert event["event_type"] == EventType.OPENED.value
        assert "open" in event["label"].lower() or "opened" in event["label"].lower()
    ...
    def test_creates_closed_event(self, event_resolver, sample_acf):
        ...
        assert len(events) == 1
        assert events[0]["event_type"] == EventType.CLOSED.value
    
    def test_creates_turned_on_event(self, event_resolver, sample_acf):
        """Test that turning on a light creates a TURNED_ON event."""
        ...
        assert len(events) == 1
        assert events[0]["event_type"] == EventType.TURNED_ON.value
    
    def test_creates_appeared_event(self, event_resolver, sample_acf):
        """Test that entity appearing creates an APPEARED event."""
        # First percept: empty
        percept1 = make_percept([])
        event_resolver.resolve(percept1, sample_acf)
        ...
        assert len(events) == 1
        assert events[0]["event_type"] == EventType.APPEARED.value
    
    def test_creates_moved_event(self, event_resolver, sample_acf):
        """Test that entity movement creates a MOVED event."""
        ...
        assert len(events) == 1
        assert events[0]["event_type"] == EventType.MOVED.value
    ...
    def test_recognizes_learned_event(self, mock_graph_store, mock_dialog_manager, sample_acf):
        ...
        known_event = GraphNode(
            ...
            extras={
                "pattern_signature": "drawer:closed->drawer:open",
                "event_type": "opened",
            },
        )
...
class TestPhase5Integration:
    ...
    def test_full_state_change_cycle(self, mock_graph_store, mock_dialog_manager):
        ...
        assert turned_on_event["event_type"] == EventType.TURNED_ON.value
        ...
        assert events3[0]["event_type"] == EventType.TURNED_OFF.value
